...
# Encoding model_a and model_b identifiers
model_tokenizer = Tokenizer()
model_tokenizer.fit_on_texts(df['model_a'].tolist() + df['model_b'].tolist())
X_model_a = np.array(model_tokenizer.texts_to_sequences(df['model_a'])).reshape(-1)
X_model_b = np.array(model_tokenizer.texts_to_sequences(df['model_b'])).reshape(-1)
...
X_train_prompt, X_test_prompt, X_train_response_a, X_test_response_a, X_train_response_b, X_test_response_b, X_train_model_a, X_test_model_a, X_train_model_b, X_test_model_b, y_train, y_test = train_test_split(
...
# Model A input
input_model_a = Input(shape=(1,))
embedding_model_a = Embedding(input_dim=len(model_tokenizer.word_index) + 1, output_dim=embedding_dim)(input_model_a)
flat_model_a = Dense(64, activation="relu")(embedding_model_a)

# Model B input
input_model_b = Input(shape=(1,))
embedding_model_b = Embedding(input_dim=len(model_tokenizer.word_index) + 1, output_dim=embedding_dim)(input_model_b)
flat_model_b = Dense(64, activation="relu")(embedding_model_b)
...
# Train the model
model.fit([X_train_prompt, X_train_response_a, X_train_response_b, X_train_model_a, X_train_model_b], y_train,
